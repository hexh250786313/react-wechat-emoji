    ...
    case 'addRecentUsed':
      {
        ...
        var foundIndex = prevState.recentUsed.findIndex(function (item, order) {
          return order && item.id === action.payload.id;
        });
        if (foundIndex !== -1) {
...
function Emoji(props) {
  var _useReducer = (0, _react.useReducer)(emojiReducer, emojiInitialState),
      _useReducer2 = _slicedToArray(_useReducer, 2),
      state = _useReducer2[0],
      dispatch = _useReducer2[1];

  var handleClick = function handleClick(payload) {
    dispatch({ type: 'addRecentUsed', payload: payload });
  ...
  return _react2.default.createElement(
    'div',
    { style: { height: props.height + 'px' }, className: 'area' },
    _react2.default.createElement(
      'div',
      { className: 'list' },
      state.recentUsed.length && _react2.default.createElement(
